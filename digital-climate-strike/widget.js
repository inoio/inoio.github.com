(function () {
  ...
  var fullPageDisplayStartDate =
    options.fullPageDisplayStartDate || new Date(2021, 2, 19); // March 19th, 2021
  var forceFullPageWidget = !!options.forceFullPageWidget;
  var cookieExpirationDays = parseFloat(options.cookieExpirationDays || 1);
  var alwaysShowWidget = !!(
    options.alwaysShowWidget ||
    window.location.hash.indexOf("ALWAYS_SHOW_DIGITAL_CLIMATE_STRIKE") !== -1
  );
  ...
  function getIframeSrc() {
    ...
    showCloseButtonOnFullPageWidget &&
      urlParams.push(["showCloseButtonOnFullPageWidget", "true"]);
  ...
  function getLanguage() {
    ...
    if (
      options.language === "es" ||
      (!options.language && navigator && navigator.language.match(/^es/))
    ) {
      language = "es";
    }
    ...
    if (
      options.language === "de" ||
      (!options.language && navigator && navigator.language.match(/^de/))
    ) {
      language = "de";
    }
    ...
    if (
      options.language === "cs" ||
      (!options.language && navigator && navigator.language.match(/^cs/))
    ) {
      language = "cs";
    }
    ...
    if (
      options.language === "fr" ||
      (!options.language && navigator && navigator.language.match(/^fr/))
    ) {
      language = "fr";
    }
    ...
    if (
      options.language === "nl" ||
      (!options.language && navigator && navigator.language.match(/^nl/))
    ) {
      language = "nl";
    }
    ...
    if (
      options.language === "tr" ||
      (!options.language && navigator && navigator.language.match(/^tr/))
    ) {
      language = "tr";
    }
    ...
    if (
      options.language === "pt" ||
      (!options.language && navigator && navigator.language.match(/^pt/))
    ) {
      language = "pt";
    }
    ...
    if (
      options.language === "it" ||
      (!options.language && navigator && navigator.language.match(/^it/))
    ) {
      language = "it";
    }
  ...
  function iFrameShouldNotBeShown() {
    ...
    return (
      (footerDisplayStartDate.getTime() > NOW &&
        fullPageDisplayStartDate.getTime() > NOW) ||
  ...
  function initializeInterface() {
    ...
    injectCSS(
      ...
      "#" +
        DOM_ID +
        " { position: fixed; right: 0; left: 0; bottom: 0px; width: 100%; height: " +
        iFrameHeight +
        "; z-index: 20000; -webkit-overflow-scrolling: touch; overflow: hidden; } " +
        "#" +
        DOM_ID +
